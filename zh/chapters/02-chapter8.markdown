...
纯粹的指令只有 `96-84=12` 个字节了，还有办法再减少目标文件的大小么？如果看了参考资料 [\[1\]][1]，看样子你又要蠢蠢欲动了：这 24 个字节是否可以插入到文件头部或程序头部？如果可以那是否意味着还可减少可执行文件的大小呢？现在来比较一下这三部分的十六进制内容。
...
然后可以参考 `X86` 的指令编码手册（也可以写成汇编生成可执行文件后用 `hexdump` 查看），可以把 `jmp` 指令编码为： `0xeb0x04` 。
...
如果想进一步研究，请阅读参考资料 [\[1\]][1]，它更深层次地讨论了 `ELF` 文件，特别是 Linux 系统对 `ELF` 文件头部和程序头部的解析。
