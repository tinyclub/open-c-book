...
从上面结果发现ELF文件头部和程序头部还有好些空洞(0)，是否可以通过引入跳转指令把12个字节分散放入到那些空洞里或者是直接覆盖掉那些系统并不关心的内容？抑或是把代码压缩以后放入可执行文件中，并在其中实现一个解压缩算法？还可以是通过一些代码覆盖率测试工具(gcov,prof)对你的代码进行优化？这个作为我们共同的练习吧！具体答案请参考附录一。
